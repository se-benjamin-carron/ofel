...
namespace ofel.Core
{
    ...
    public sealed class FiniteElementStructure
    {
        ...
        public List<PointStructureData> PointsStructureData = new List<PointStructureData>();
        ...
        public Dictionary<string, MathNet.Numerics.LinearAlgebra.Vector<double>> ForceOnFreeDirections
        {
            get; set;
        } = new Dictionary<string, MathNet.Numerics.LinearAlgebra.Vector<double>>();
        public Dictionary<string, MathNet.Numerics.LinearAlgebra.Vector<double>> ForceOnFixedDirections
        {
            get; set;
        } = new Dictionary<string, MathNet.Numerics.LinearAlgebra.Vector<double>>();
        public Dictionary<string, MathNet.Numerics.LinearAlgebra.Vector<double>> DisplacementsOnFreeDirections
        {
            get; set;
        } = new Dictionary<string, MathNet.Numerics.LinearAlgebra.Vector<double>>();

        public Dictionary<string, MathNet.Numerics.LinearAlgebra.Vector<double>> AllDisplacementsCase
        {
            get; set;
        } = new Dictionary<string, MathNet.Numerics.LinearAlgebra.Vector<double>>();
        public Dictionary<string, Dictionary<string, MathNet.Numerics.LinearAlgebra.Vector<double>>> AllDisplacementsCombinations
        {
            get; set;
        } = new Dictionary<string, Dictionary<string, MathNet.Numerics.LinearAlgebra.Vector<double>>>();
        public Dictionary<string, MathNet.Numerics.LinearAlgebra.Vector<double>> SupportReactions
        {
            get; set;
        } = new Dictionary<string, MathNet.Numerics.LinearAlgebra.Vector<double>>();
        ...
        public FiniteElementStructure(List<FiniteElementMember> f_e_member, double mesh, BuildingUse use, double altitude = 0.0)
        {
            FiniteElementMembers = f_e_member ?? throw new ArgumentNullException(nameof(f_e_member));
            if (mesh <= 0) throw new ArgumentOutOfRangeException(nameof(mesh), "mesh must be > 0");
            Mesh = mesh;
        ...
        public void Compute(bool IsSecondOrder = false)
        {
            ...
            {
                ForceOnFreeDirections[loading_case] = AssemblyMatrixClass.AssembleVector(FreeDirectionsMatrix, loading);
                ForceOnFixedDirections[loading_case] = AssemblyMatrixClass.AssembleVector(FixedDirectionsMatrix, loading);
            }
            SolveResults();
        ...
        public void ComputeAllPoints()
        {
            ...
            foreach (FiniteElementMember f_e_member in FiniteElementMembers)
            {
                ...
                (member_points_data, pointsAllStructure) = f_e_member.GetPointsStructureData(pointsAllStructure);
                PointsStructureDataByMember.Add(member_points_data);
            }
            PointsStructureData = pointsAllStructure;
        }
        ...
        public void ComputeAllForce()
        {
            ...
            for (int memberIndex = 0; memberIndex < FiniteElementMembers.Count; memberIndex++)
            {
                ...
                foreach (var (loading_case, force) in f_e_member.EffortsByLoadCase)
                {
                    MathNet.Numerics.LinearAlgebra.Vector<double> structureForce = AssemblyMatrixClass.AssembleVector(assembly_matrix, force);
        ...
        public void ComputeAllAssemblyMatrix()
        {
            ...
            int totalGlobalDofs = PointStructureData.nextId * 6;
            for (int mi = 0; mi < FiniteElementMembers.Count; mi++)
            {
                ...
                for (int k = 0; k < member.MainEpsilons.Count; k++)
                {
                    var me = member.MainEpsilons[k];
                    // find corresponding index in AllEpsilons
                    int nodeIndex = member.AllEpsilons.FindIndex(a => Math.Abs(a.Epsilon - me.Epsilon) < 1e-9);
                ...
                var A = MathNet.Numerics.LinearAlgebra.Matrix<double>.Build.Dense(totalGlobalDofs, memberDofs, 0.0);
        ...
        {
            Matrix<double> massMatrix = Matrix<double>.Build.Dense(PointStructureData.nextId * 6, PointStructureData.nextId * 6);
            Matrix<double> stiffnessMatrix = Matrix<double>.Build.Dense(PointStructureData.nextId * 6, PointStructureData.nextId * 6);
            for (int memberIndex = 0; memberIndex < FiniteElementMembers.Count; memberIndex++)
            {
                var member = FiniteElementMembers[memberIndex];
                var assembly_matrix = AssemblyMatrices[memberIndex];
                Matrix<double> k_member = AssemblyMatrixClass.AssembleMatrix(assembly_matrix, member.GlobalStiffnessMatrix);
                Matrix<double> m_member = AssemblyMatrixClass.AssembleMatrix(assembly_matrix, member.GlobalMassMatrix);
        ...
        {
            int size = PointStructureData.nextId * 6;
            ...
            foreach (PointStructureData point in PointsStructureData)
            {
                FixedDirections.AddRange(point.FixedDirectionsIndices());
                FreeDirections.AddRange(point.FreeDirectionsIndices());
                if (point.IsSupport)
                {
                    // Build a small assembly matrix that maps the point's 6 local DOFs
                    // into the global DOF indices. Ensure `size` is large enough to
                    // contain the global indices returned by SupportIndices(). If not,
                    // enlarge the local size to avoid out-of-range exceptions.
                    var supportIdx = point.SupportIndices();
                    if (supportIdx.Count > 0)
                    {
                        int maxIdx = supportIdx.Max();
                        if (maxIdx >= size)
                        {
                            // enlarge local size to accommodate the indices
                            size = maxIdx + 1;
                        }
                    }
                    SupportMatrices[point.id] = AssemblyMatrixClass.GetAssemblyMatrix(size, supportIdx);
                }
            }
        ...
        {
            // Kus should be reduced stiffness on free DOFs: A_free^T * K_global * A_free
        ...
        public Matrix<double> GetKssMatrix()
        {
            ...
            return AssemblyMatrixClass.DisAssembleMatrix(FixedDirectionsMatrix, GlobalStiffnessMatrix);
        }

        public void SolveResults()
        {
            Dictionary<string, MathNet.Numerics.LinearAlgebra.Vector<double>> displacementsByLoadCase = new Dictionary<string, MathNet.Numerics.LinearAlgebra.Vector<double>>();
            foreach (var (loading_case, force) in ForceOnFreeDirections)
            {
                MathNet.Numerics.LinearAlgebra.Vector<double> displacements = KusMatrix.Solve(force);
                displacementsByLoadCase[loading_case] = displacements;
            }
            DisplacementsOnFreeDirections = displacementsByLoadCase;
        }

        public void ComputeSupportReactions()
        {
            MathNet.Numerics.LinearAlgebra.Vector<double> supportReactions = MathNet.Numerics.LinearAlgebra.Vector<double>.Build.Dense(FixedDirectionsMatrix.RowCount);
            foreach (var (loading_case, displacements) in DisplacementsOnFreeDirections)
            {
                MathNet.Numerics.LinearAlgebra.Vector<double> reaction = KssMatrix * displacements;
                reaction += ForceOnFixedDirections[loading_case];
                SupportReactions[loading_case] = reaction;
            }
        }
        ...
        public void ComputeAllDisplacements()
        {
            ...
            var vector_fixed_direction = MathNet.Numerics.LinearAlgebra.Vector<double>.Build.Dense(size);
            Dictionary<string, MathNet.Numerics.LinearAlgebra.Vector<double>> allDisplacements = new Dictionary<string, MathNet.Numerics.LinearAlgebra.Vector<double>>();
            foreach (var (loading_case, displacements) in DisplacementsOnFreeDirections)
            {
                MathNet.Numerics.LinearAlgebra.Vector<double> displacements_fixed = AssemblyMatrixClass.AssembleVector(FixedDirectionsMatrix, vector_fixed_direction);
                MathNet.Numerics.LinearAlgebra.Vector<double> displacements_free = AssemblyMatrixClass.AssembleVector(FreeDirectionsMatrix, displacements);
        ...
        {
            Dictionary<string, MathNet.Numerics.LinearAlgebra.Vector<double>> GlobalDisplacementsUlsF = CombineAllDisplacementsCombinations(
            ...
            Dictionary<string, MathNet.Numerics.LinearAlgebra.Vector<double>> GlobalDisplacementsUlsAcc = CombineAllDisplacementsCombinations(
            ...
            Dictionary<string, MathNet.Numerics.LinearAlgebra.Vector<double>> GlobalDisplacementsSlsCar = CombineAllDisplacementsCombinations(
            ...
            Dictionary<string, MathNet.Numerics.LinearAlgebra.Vector<double>> GlobalDisplacementsSlsQuasiPermanent = CombineAllDisplacementsCombinations(
                AllDisplacementsCase, LoadCoefficient.Sls.Permanent, LoadCoefficient.Sls.Variable, GetPsi2Coefficients(), GetPsi2Coefficients());
            Dictionary<string, MathNet.Numerics.LinearAlgebra.Vector<double>> GlobalDisplacementsSlsFrequent = CombineAllDisplacementsCombinations(
        ...
        public Dictionary<string, MathNet.Numerics.LinearAlgebra.Vector<double>> CombineAllDisplacementsCombinations(
            Dictionary<string, MathNet.Numerics.LinearAlgebra.Vector<double>> cases, double[] coefPerm, double[] coefVar,
            Dictionary<string, double> psiMinors, Dictionary<string, double> psiMajor)
        {
            var permanents = cases.Where(kv => kv.Key == "G" || kv.Key == "PP").ToList();
            var variables = cases.Where(kv => kv.Key != "G" && kv.Key != "PP").ToList();

            var combinaisonDict = new Dictionary<string, MathNet.Numerics.LinearAlgebra.Vector<double>>();
            int vectorSize = cases.Count > 0 ? cases.First().Value.Count : 0;
            MathNet.Numerics.LinearAlgebra.Vector<double> G = cases.ContainsKey("G") ? cases["G"] : MathNet.Numerics.LinearAlgebra.Vector<double>.Build.Dense(vectorSize, 0.0);
            MathNet.Numerics.LinearAlgebra.Vector<double> OW = cases.ContainsKey("OW") ? cases["OW"] : MathNet.Numerics.LinearAlgebra.Vector<double>.Build.Dense(vectorSize, 0.0);
            MathNet.Numerics.LinearAlgebra.Vector<double> combinationPerm = G + OW;
            ...
            foreach (var coefPermanent in coefPerm)
            {
                foreach (var coefVariable in coefVar)
                {
                    ...
                    {
                        MathNet.Numerics.LinearAlgebra.Vector<double> combinationVar = MathNet.Numerics.LinearAlgebra.Vector<double>.Build.Dense(
                        ...
                        string variable_name = $"+ {major_psi * coefVariable}x{major}";
                        ...
                        foreach (var minor in minors)
                        {
                            ...
                            variable_name += minor + $" + {Math.Round(coefVariable * minor_psi, 2)} {minor_psi} ";
                        }
                        if (skip) continue;
                        string name = $"{numCombinaison}/ {coefPermanent} (G + PP) + {Math.Round(coefVariable * major_psi, 2)}x{major} + {variable_name}";
                        MathNet.Numerics.LinearAlgebra.Vector<double> combinationTotal = combinationVar * coefVariable + combinationPerm * coefPermanent;
